...
import { captureShelfLocation } from "@/utils/location";
...
export default function ShelfSetup() {
  ...
  const { shelf, shelves, setupShelf, setShelfLocation } = useLibrary();
  ...
  const [gpsStatus, setGpsStatus] = useState<"idle" | "saved" | "denied" | "unavailable">("idle");
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const gpsPromise = captureShelfLocation();
      ...
      await setupShelf(targetShelf.id, name.trim(), result.mapId, null);
      void gpsPromise.then(async (gps) => {
        setGpsStatus(gps.status);
        if (gps.location) {
          await setShelfLocation(targetShelf.id, result.mapId!, gps.location);
        }
      }).catch(() => setGpsStatus("unavailable"));
      Alert.alert(
        "Shelf saved",
        "The shelf map is stored locally. GPS will be added when a nearby reading is available, but AR can find the exact shelf without it.",
        [
  ...
  return (
    <Screen>
      ...
      />
      <View style={[styles.gpsNotice, { backgroundColor: colors.secondary, borderColor: colors.border }]}>
        <Feather name="map-pin" size={17} color={colors.primary} />
        <Text style={[styles.gpsText, { color: colors.mutedForeground }]}>
          {gpsStatus === "saved"
            ? "GPS area saved with this shelf."
            : gpsStatus === "denied"
              ? "GPS permission was denied. The AR shelf map was still saved."
              : gpsStatus === "unavailable"
                ? "GPS is unavailable here. The AR shelf map can still be used."
                : targetShelf.location
                  ? "A GPS area is saved. AR will handle the exact shelf position."
                  : "GPS helps identify the right room or area; AR handles the exact shelf position."}
        </Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  gpsNotice: { borderWidth: 1, borderRadius: 15, padding: 12, flexDirection: "row", alignItems: "center", gap: 9 },
  gpsText: { flex: 1, fontSize: 12, lineHeight: 18 },
