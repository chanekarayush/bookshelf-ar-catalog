  ...
  return (
    <Screen>
      ...
      <View style={[styles.shelfCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <View style={styles.row}><Text style={[styles.small, { color: colors.mutedForeground }]}>{placed} of {books.length} placed · {shelf.location ? "GPS saved" : "GPS not set"}</Text><Pressable onPress={() => router.push("/shelf")}><Text style={[styles.link, { color: colors.primary }]}>{shelf.mapped ? "Re-scan shelf" : "Set up shelf"} <Feather name="arrow-up-right" size={13} /></Text></Pressable></View>
      </View>
