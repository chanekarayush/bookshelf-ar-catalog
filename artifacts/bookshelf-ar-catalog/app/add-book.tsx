...
import { canonicalizeIsbn, useLibrary } from "@/context/LibraryContext";
...
export default function AddBook() {
  ...
  const lookup = async (isbn: string) => {
    ...
    try {
      ...
      Alert.alert("Book not found", "Enter the book details manually before saving.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onBarcode = ({ data }: { data: string }) => {
    const isbn = canonicalizeIsbn(data);
    if (!isbn) return;
    setScanning(false);
    set("isbn", isbn);
    void lookup(isbn);
  ...
  const save = async () => {
    Keyboard.dismiss();
    const isbn = canonicalizeIsbn(form.isbn);
    if (!isbn || !form.title.trim() || !form.authors.trim()) {
      Alert.alert("Almost there", "A valid ISBN-10 or ISBN-13, title, and author are required.");
      return;
    }
    const existing = books.find((book) => canonicalizeIsbn(book.isbn) === isbn);
    const book = await saveBook({ ...form, isbn, title: form.title.trim(), authors: form.authors.trim(), id: existing?.id });
    router.replace({ pathname: "/assign-book", params: { id: book.id } });
  };
